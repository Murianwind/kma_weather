...
class KMAApiClient:
                ...
                for item in items:
                    ...
                    if f_date == tomorrow_str:
                        if f_time == "0900" and cat == "SKY":
                            data["weather_am_tomorrow"] = self._sky_to_text(val, data.get("PTY", "0"))
                        if f_time == "1500" and cat == "SKY":
                            data["weather_pm_tomorrow"] = self._sky_to_text(val, data.get("PTY", "0"))
    ...
    def _sky_to_text(self, sky, pty):
        if pty != "0":
            return {"1": "비", "2": "비/눈", "3": "눈", "4": "소나기"}.get(pty, "강수")
