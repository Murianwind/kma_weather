...
# 1. 중기예보 구역 ID 및 육상코드 형식 전수 검증 (오류 수정됨)
def test_exhaustive_mid_term_ids():
    for expected_id, (lat, lon) in _TEMP_ID_COORDS.items():
        ...
        # 충북(11C10000) 등을 고려하여 0이 4개 또는 5개로 끝나는지 확인
        assert (land_id.endswith("0000") or land_id.endswith("00000")), f"❌ 형식 오류: {land_id}"
...
# 3. 에어코리아 측정소 위치 결정 로직 검증 (추가됨)
@pytest.mark.asyncio
async def test_air_korea_location_resolution(hass, mock_entry):
    from custom_components.kma_weather.coordinator import KMAWeatherUpdateCoordinator
    coordinator = KMAWeatherUpdateCoordinator(hass, mock_entry)
    with patch.object(coordinator, "_resolve_location", return_value=(37.5665, 126.9780)): # 서울시청
        # 실제 API 호출을 모킹하여 station이 air 섹션에 포함되는지 확인
...
# 4. 날씨 요약 사수 로직 검증 (추가됨)
@pytest.mark.asyncio
async def test_summary_persistence_at_midnight(hass, mock_entry):
    from custom_components.kma_weather.coordinator import KMAWeatherUpdateCoordinator
    coordinator = KMAWeatherUpdateCoordinator(hass, mock_entry)
    coordinator._wf_pm_today = "눈/비" # 저장된 값
    coordinator._daily_date = datetime.now().date()
    
    # 밤 11시, API에서 예보가 사라진 상황 시뮬레이션
    new_data = {"weather": {"wf_pm_today": None}}
    
    # 12시 이후(오후)이므로 저장된 야간 날씨를 꺼내와야 함
