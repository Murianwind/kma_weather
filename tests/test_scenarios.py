...
async def test_kma_full_scenarios(hass, mock_config_entry, kma_api_mock_factory, freezer):
    ...
    # 예보 데이터가 현재 시간을 기준으로 정상적으로 필터링 되는지 확인
    assert forecast[0]["datetime"].startswith("2026-04-09")

    # --- 시나리오 5 & 6. 현재 위치 출력 및 변경 시 갱신 ---
    # 5. 초기 위치 확인 (MOCK 데이터가 '경기도 화성시'를 주는지 확인)
    assert hass.states.get(f"sensor.{p}_location").state == "경기도 화성시"

    # 6. 위치 변경 시뮬레이션
    # api_kma.py의 KMAWeatherAPI 클래스 내 _get_address 메서드를 직접 패치
    with patch("custom_components.kma_weather.api_kma.KMAWeatherAPI._get_address", new_callable=AsyncMock) as mock_addr:
        ...
        # 코디네이터 리프레시를 통해 새 위치 정보 반영 유도
        coordinator = hass.data[DOMAIN][mock_config_entry.entry_id]
        await coordinator.async_refresh()
        await hass.async_block_till_done()

        state = hass.states.get(f"sensor.{p}_location")
        assert state.state == "부산광역시"
